    ...
    <v-card>
      <v-card-title justify-center>
        <v-row justify="center" class="mt-1" >
          <span align="center" justify="center">Edit อำเภอ / เขต(Distirct)</span>
        </v-row>
      </v-card-title>
      <v-card-text>
...
export default {
  ...
  props: {
    ...
    district: {type: District, reqtured: true },
  },
  data() {
    return {
      isLoading: false,
  ...
  created() {
    this.form.DistrictName = this.district.DistrictName;
  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        ...
        const result = await this.editDistrict(modifyDistrict);
        console.log(result);
