<template>
  <div class="container">
    ...
    <h2 class="text-center mt-2 mb-4">Address : ตำบล / แขวง (Sub District)</h2>
    <!-- Search filter -->
    <v-form ref="form">
      <v-row class="mx-1">
        <v-col cols="6" sm="4">
    ...
    <AddSubDistrict v-if="addModal" @close="handleCloseAdd" :isDialogOpen="addModal" />
    <EditSubDistrict v-if="isEdit" @close="handleCloseEdit" :isDialogOpen="isEdit" :subDistrict="editingItem" />
    <!-- <v-dialog v-model="deleteDialog" max-width="500px">
      <v-card>
        <v-card-title class="headline">Confirm Delete</v-card-title>
        <v-card-text>Are you sure you want to delete this item?</v-card-text>
...
import AddSubDistrict from './components/AddSubDistrict.vue';
import EditSubDistrict from './components/EditSubDistrict.vue';

export default {
  name: 'SubDistrictPage',
  components: {
    AddSubDistrict,
    EditSubDistrict,
  },
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'Sub District name', value: 'SubDistrictName', sortable: false },
        { text: 'Sub District Id', value: 'SubDistrictId', sortable: false },
  ...
  computed: {
    ...
    filteredSubDistricts() {
      return this.subDistricts.filter(_subDistrict => {
        const searchTerm = this.filterKeyword.toLowerCase();
        return _subDistrict.SubDistrictName.toLowerCase().includes(searchTerm);
      });
    }
  },
  created() {
  ...
  methods: {
    ...mapActions('master-data', {
      ...
      fetchSubDistrict: 'fetchSubDistricts',
    ...
    async search() {
      ...
      this.loading = true;
      try {
        let responseData = await this.fetchSubDistrict(this.selectedDistrict);
        console.log(responseData)
        let listSubDistrict = responseData.map(item => new SubDistrict({
          SubDistrictId: item.SubDistrictId ?? null,
          SubDistrictName: item.SubDistrictName ?? null,
          ZipCode: item.ZipCode ?? null,
        }));
        this.subDistricts = listSubDistrict;
        this.totalItems = this.subDistricts.length;
      } catch (error) {
